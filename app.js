...
const syncPassword = document.querySelector("#sync-password");
const syncStatus = document.querySelector("#sync-status");
const syncSignup = document.querySelector("#sync-signup");
...
taskForm.addEventListener("submit", async (event) => {
  ...
  const task = createTask(title, taskCategory.value, taskIcon.value, taskTopic.value);
  tasks.unshift(task);
  taskForm.reset();
  saveTasks();
  render();
...
function showSyncForm(show) {
  syncForm?.classList.toggle("is-hidden", !show);
  if (syncSignup) syncSignup.hidden = !show;
}
...
async function syncTask(task) {
  ...
  try {
    ...
    const saved = await upsertRemoteTask({ userId: currentUser.id, task });
...
function mergeRemoteTasks(remoteTasks) {
  ...
  const mergedRemote = remoteTasks.map((remoteTask) => {
    ...
    return existing ? { ...existing, ...remoteTask, pointsAwarded: existing.pointsAwarded || remoteTask.done } : { ...remoteTask, pointsAwarded: remoteTask.done };
  });
  ...
  tasks = normalizeTasks([...mergedRemote, ...localOnly]);
  saveTasks();
  render();
}
...
async function initializeBloomSync() {
  ...
  syncForm?.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      const user = await signInWithEmail({ email: syncEmail.value.trim(), password: syncPassword.value });
  ...
  syncSignup?.addEventListener("click", async () => {
    try {
      ...
      const user = await signUpWithEmail({ email: syncEmail.value.trim(), password: syncPassword.value });
      setSyncStatus(user ? "Account created." : "Please check your confirmation email.");
  ...
  onAuthChange(async (user) => {
    ...
    if (!user) {
      ...
      isRemoteReady = false;
      showSyncForm(true);
      setSyncStatus("Bloom OSと同じアカウントで同期できます。");
    }
  });
...
async function addTaskFromTemplate(id) {
  const template = taskTemplates.find((item) => item.id === id);
  if (!template) return;
  const task = createTask(template.title, template.category, template.icon, template.topic);
  tasks.unshift(task);
  saveTasks();
  render();
  await syncTask(task);
